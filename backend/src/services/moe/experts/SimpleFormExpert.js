...
Return ONLY valid JSON matching the exact output format from the knowledge base.`;
    ...
    const processedForms = (result.forms || []).map(form => {
      if (!form.gridLayout && form.fields) {
        console.log(`[SimpleFormExpert] Auto-generating gridLayout for form: ${form.name}`);
    ...
    if (onThinking) {
      onThinking({
        ...
        content: `Generated ${processedForms.length} simple form(s)`
      });
    }
  ...
  generateGridLayout(fields, designSystem) {
    ...
    const layoutColumns = designSystem?.layout?.columns?.desktop || 1; // Simple forms default to 1 column
